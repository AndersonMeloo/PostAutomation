      ...
      <header className="dashboard-card relative overflow-hidden p-6 md:p-8">
        ...
        <div className="relative">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3 text-sm">
            <span className="dash-panel rounded-full border px-3 py-1.5 font-mono text-foreground">
              Base: {moduleData.basePath}
            </span>
            <span className="dash-panel rounded-full border px-3 py-1.5 text-foreground">
      ...
      <div className="grid gap-3">
        {moduleData.endpoints.map((endpoint) => (
          <article
            ...
            className="premium-surface-soft p-4 transition duration-200 hover:-translate-y-0.5 hover:border-(--border-soft-hover)"
          >
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-muted">{endpoint.note}</p>
              </div>

              <span className="dash-panel w-fit rounded-full border px-2.5 py-1 text-xs uppercase tracking-[0.16em] text-muted">
                {endpoint.access}
              </span>
            </div>
          </article>
